...
one positional argument which is expected to be a dictionary 
...
expected to be an object (not a group of objects), and an arbitrary number
of named arguments, that are meant to specify styling options. 
...
The renderer also takes an arbitrary number of named arguments, 
meant to specify drawing options, that will be passed as they are to 
...
#let renderer(dictionary-of-drawing-functions) = (..args) => {
  ...
  return objects.map(obj => {
    ...
    return dictionary-of-drawing-functions.at(obj("type"))(obj, ..style)
  })
}